<template>
  <header class="header">
    <!-- Left section -->
    <div class="header__left">
      <div class="header__icon-wrapper tooltip-right" :data-tooltip="$t('header.allApps')">
        <div class="header__menu-icon"></div>
      </div>
      <div class="header__logo"></div>
      <span class="header__app-name">{{ $t('app.name') }}</span>
    </div>

    <!-- Right section -->
    <div class="header__right">
      <div class="header__company" :data-tooltip="$t('header.company')">
        <span class="company-name">{{ $t('header.company') }}</span>
        <i class="chevron"></i>
      </div>

      <div class="header__right-setting">
        <div :data-tooltip="$t('header.exportExcel')">
          <div class="header__icon-btn header__icon-download"></div>
        </div>
        <div class="nav-separator"></div>
        <div :data-tooltip="$t('header.inventoryLookup')">
          <div class="header__icon-btn header__icon-package"></div>
        </div>
        <div :data-tooltip="$t('header.inviteMisa')">
          <div class="header__icon-btn header__icon-user"></div>
        </div>
        <div :data-tooltip="$t('header.smartReminder')">
          <div class="header__icon-btn header__icon-reminder"></div>
        </div>
        <div :data-tooltip="$t('header.notification')">
          <div class="header__icon-btn header__icon-noti"></div>
        </div>
        <div :data-tooltip="$t('header.help')">
          <div class="header__icon-btn header__icon-help"></div>
        </div>
        <div :data-tooltip="$t('header.moreFeatures')">
          <div class="header__icon-btn header__icon-more"></div>
        </div>
        <div :data-tooltip="$t('header.knowledge')">
          <div class="header__icon-btn header__icon-knowledge"></div>
        </div>
        <div class="header__avatar">
          <img
            src="https://amisplatform.misacdn.net/APIS/PlatformAPI/api/Avatar/154e11d2-cf4c-44ad-9b8b-9ed40778bf40/jq8vew6b.jpg?avatarID=7c88d81f-ce78-4177-acd0-312a3d27378f&width=64&height=64"
            alt=""
          />
        </div>
      </div>
    </div>
  </header>
</template>

<script setup></script>

<style scoped>
/* Giữ nguyên toàn bộ CSS gốc */
.header {
  position: fixed;
  top: 0;
  left: 0;
  right: 0;
  z-index: 999;
  height: 48px;
  background-color: var(--bg-page);
  border-bottom: 1px solid #d1d5db;
  display: flex;
  align-items: center;
  padding: 0 16px;
  gap: 12px;
  width: 100%;
  font-family: Inter;
}
.header__left {
  max-width: 200px;
  display: flex;
  gap: 16px;
  align-items: center;
  height: 100%;
}
.header__menu-icon {
  mask-position: 0px -35px;
  cursor: pointer;
  height: 24px;
  width: 24px;
  min-height: 24px;
  min-width: 24px;
  position: relative;
  -webkit-mask-repeat: no-repeat;
  background-color: #4b5563;
  -webkit-mask-image: url(https://qtsxcdng2.misacdn.net/assets/pas.Icon%20Warehouse-e29a964d.svg?v=10.1.2.4);
}
.header__logo {
  background-image: url(https://qtsxcdng2.misacdn.net/assets/pas.ic_production_100px-626a0b65.svg?v=10.1.2.4);
  width: 30px;
  height: 30px;
  background-repeat: no-repeat;
  background-size: contain;
}
.header__app-name {
  font-size: var(--font-size-lg);
  font-weight: 700;
  color: var(--text-heading);
  white-space: nowrap;
  margin-left: -4px;
}
.header__company {
  display: flex;
  align-items: center;
  font-size: var(--font-size-base);
  color: var(--text-heading);
  cursor: pointer;
  white-space: nowrap;
  font-weight: 700;
  max-width: 440px;
  overflow: hidden;
  position: relative;
  text-overflow: ellipsis;
  gap: 4px;
}
.company-name {
  white-space: nowrap;
  overflow: hidden;
  text-overflow: ellipsis;
  display: block;
}
.header__company .chevron {
  font-size: 10px;
  color: var(--text-heading);
  mask-position: -32px 0px;
  height: 16px;
  width: 16px;
  min-height: 16px;
  min-width: 16px;
  position: relative;
  -webkit-mask-repeat: no-repeat;
  background-color: #4b5563;
  -webkit-mask-image: url(https://qtsxcdng2.misacdn.net/assets/pas.Icon%20Warehouse-e29a964d.svg?v=10.1.2.4);
}
.header__right {
  height: 100%;
  display: flex;
  align-items: center;
  justify-content: space-between;
  flex: 1;
  margin-left: 46px;
}
.header__right-setting {
  display: flex;
  align-items: center;
  gap: 11.5px;
}
.header__icon-btn {
  height: 24px;
  width: 24px;
  min-height: 24px;
  min-width: 24px;
  position: relative;
  -webkit-mask-repeat: no-repeat;
  background-color: #4b5563;
  cursor: pointer;
}
.header__icon-download {
  mask-position: -744px -36px;
  -webkit-mask-image: url(https://qtsxcdng2.misacdn.net/assets/pas.sprite_awh-bc818426.svg?v=10.1.2.4);
}
.nav-separator {
  width: 1px;
  height: 16px;
  background: #d1d5db;
  align-self: center;
  border-radius: 1px;
}
.header__icon-package {
  background: url(https://qtsxcdng2.misacdn.net/assets/pas.ic_package-e7cf3c47.svg) no-repeat;
}
.header__icon-user {
  width: 32px;
  height: 32px;
  position: relative;
  border-radius: 16px;
  background-position: center;
  background-size: 24px;
  background-repeat: no-repeat;
  overflow: unset;
  background-color: var(--bg-white);
  cursor: pointer;
  background-image: url(data:image/svg+xml;base64,);
}
.header__icon-user:hover {
  background-color: rgba(0, 0, 0, 0.1);
}
.header__icon-reminder {
  background-position: -552px -428px;
  background-color: var(--bg-white);
  background-image: url(https://qtsxcdng2.misacdn.net/assets/pas.Sprites-1d4aa31f.svg?v=10.1.2.4);
}
.header__icon-noti {
  mask-position: -24px -36px;
  -webkit-mask-image: url(https://qtsxcdng2.misacdn.net/assets/pas.Icon%20Warehouse-e29a964d.svg?v=10.1.2.4);
}
.header__icon-help {
  mask-position: -48px -36px;
  -webkit-mask-image: url(https://qtsxcdng2.misacdn.net/assets/pas.Icon%20Warehouse-e29a964d.svg?v=10.1.2.4);
}
.header__icon-more {
  mask-position: -8px -428px;
  -webkit-mask-image: url(https://qtsxcdng2.misacdn.net/assets/pas.qtsx_icon-e5768799.svg?v=10.1.2.4);
}
.header__icon-knowledge {
  border-radius: 16px;
  background-position: center;
  background-size: 24px;
  background-repeat: no-repeat;
  overflow: unset;
  cursor: pointer;
  width: 32px;
  height: 32px;
  position: relative;
  background-color: var(--bg-white);
  background-image: url(data:image/svg+xml;base64,);
}
.header__avatar {
  border-radius: 50%;
  cursor: pointer;
  width: 32px;
  height: 32px;
  overflow: hidden;
  margin-left: 15px;
}
.header__avatar img {
  width: 100%;
  height: 100%;
  max-width: 32px;
  max-height: 32px;
}
</style>
